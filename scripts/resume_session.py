...
def parse_args():
    parser = argparse.ArgumentParser(description="Resolve the next action for a SUPERLOOPER task.")
...
def write_feedback_report(workspace_root, state, normalized):
    ...
    report_path = workspace_root / ".superlooper" / "reports" / task_id / filename
    ...
    return f".superlooper/reports/{task_id}/{filename}", rollback_target_phase
...
def validate_execution_summary_approval(workspace_root, state):
    task_id = state["task_id"]
    expected = f".superlooper/reports/{task_id}/execution_summary.md"
    ...
    validator = ContractValidator(workspace_root, task_id)
...
def validate_execution_summary_retry(workspace_root, state):
    task_id = state["task_id"]
    errors = []
    if state.get("project_initialized") is not True:
        errors.append("project_initialized 必须为 true。")
    if state.get("ui_status") != "APPROVED" or state.get("ui_artifacts_validated") is not True:
        errors.append("UI 尚未审核通过或 ui_artifacts_validated 不是 true。")
    required_paths = [
        *[
            workspace_root / ".superlooper" / "context" / task_id / "ui" / filename
            for filename in ("ui-spec.md", "page-map.md", "interaction-flow.md", "ui-handoff.md", "preview.html")
        ],
        *[
            workspace_root / ".superlooper" / "context" / task_id / "design" / filename
            for filename in ("architecture.md", "tech-stack.md", "project-profile.md", "initialization-advice.md")
        ],
    ]
    for path in required_paths:
        if not path.exists():
            errors.append(f"执行摘要前置产物不存在：{path.relative_to(workspace_root).as_posix()}")
    validator = ContractValidator(workspace_root, task_id)
    ...
    if isinstance(alignment, dict) and alignment.get("upstream_alignment_status") != "PASS":
        decisions = alignment.get("blocking_decisions")
        if isinstance(decisions, list) and decisions:
            errors.extend(str(item) for item in decisions)
        else:
            errors.append("upstream_alignment.md 必须为 PASS。")
    return "；".join(errors) if errors else None


def validate_requirement_alignment_approval(workspace_root, state):
    task_id = state["task_id"]
    expected = f".superlooper/reports/{task_id}/requirement_alignment_report.md"
...
def read_yaml_status_report(workspace_root, state, filename):
    report_path = workspace_root / ".superlooper" / "reports" / state["task_id"] / filename
    if not report_path.exists():
        return None, f"{filename} 不存在：{report_path}"
    validator = ContractValidator(workspace_root, state["task_id"])
    ...
    try:
        data = normalize_legacy_identity(data, filename)
    except SessionStateValidationError as exc:
        return None, str(exc)
    if data.get("task_id") != state["task_id"]:
        return None, f"{filename} task_id 与当前 task_id 不一致。"
    return data, None
...
def write_upstream_alignment_feedback(workspace_root, state, alignment):
    task_id = state["task_id"]
    report_path = workspace_root / ".superlooper" / "reports" / task_id / "upstream_alignment_feedback.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    report_path.write_text(content, encoding="utf-8")
    return f".superlooper/reports/{task_id}/upstream_alignment_feedback.md"
...
def apply_upstream_alignment_loop(workspace_root, state_path, state):
    if state.get("phase_status") != "running" or state.get("current_phase") not in {"prd", "ui_design", "design", "initialization", "run", "requirement_alignment"}:
        return None
    report_path = workspace_root / ".superlooper" / "reports" / state["task_id"] / "upstream_alignment.md"
    if not report_path.exists():
        return None
    validator = ContractValidator(workspace_root, state["task_id"])
    alignment = validator.validate_upstream_alignment(required=True)
...
def apply_continue_current_flow(workspace_root, state):
    ...
    if current_phase == "design" and phase_status == "waiting_review":
        module_split = workspace_root / ".superlooper" / "manifests" / task_id / "module-split.json"
        if updated.get("ui_status") != "APPROVED" or updated.get("ui_artifacts_validated") is not True:
            updated["last_error"] = "UI 尚未审核通过或 ui_artifacts_validated 不是 true。"
            updated["next_actions"] = ["先完成 UI 审核并校验 UI 产物"]
        elif updated.get("project_initialized") is not True and updated.get("workflow_mode") == "strict_review":
            validator = ContractValidator(workspace_root, task_id)
            advice = validator.validate_initialization_advice(required=True)
            if validator.errors or not isinstance(advice, dict):
                updated["last_error"] = "；".join(validator.errors) or "initialization-advice.md 缺少机器参数块。"
                updated["next_actions"] = ["修正并校验 initialization-advice.md 后重新确认设计"]
            else:
                updated["current_phase"] = "initialization"
                updated["phase_status"] = "waiting_review"
                updated["project_category"] = None
                updated["project_version"] = None
                updated["project_root"] = advice["project_root"]
                updated["last_error"] = None
                updated["next_actions"] = ["请选择初始化项目分类：java/go/springboot/pom/lua"]
        elif updated.get("project_initialized") is not True:
            updated["phase_status"] = "blocked"
            ...
            if validator.errors:
                ...
                updated["phase_status"] = "pending"
                updated["last_error"] = None
                updated["next_actions"] = [f"运行 /spl:run {task_id} 生成执行清单和执行摘要"]
        return updated
    ...
    if current_phase == "run" and phase_status == "blocked":
        apply_report = workspace_root / ".superlooper" / "reports" / task_id / "apply_report.json"
...
def apply_user_input(workspace_root, state_path, state, user_input):
    ...
    if normalized["canonical_action"] == "prd_decisions_confirmed":
    ...
    elif normalized["canonical_action"] == "approve_and_proceed":
        updated["current_phase"] = "ui_design"
        updated["phase_status"] = "pending"
        updated["ui_status"] = "NOT_STARTED"
        updated["ui_output_dir"] = f".superlooper/context/{updated['task_id']}/ui/"
        updated["ui_artifacts_validated"] = False
        updated["last_error"] = None
        updated["next_actions"] = [f"运行 /spl:ui {updated['task_id']}"]
        ...
        if updated.get("ui_artifacts_validated") is not True:
            ...
            updated["ui_output_dir"] = updated.get("ui_output_dir") or f".superlooper/context/{updated['task_id']}/ui/"
        ...
        updated["phase_status"] = "blocked"
        updated["change_impact_report"] = None
        updated["affected_modules"] = []
        updated["rollback_target_phase"] = None
        updated["last_error"] = "深层变更等待 impact-analyzer 生成并校验 change_impact_report.md。"
    ...
    elif normalized["canonical_action"] == "retry_execution_summary":
        retry_error = validate_execution_summary_retry(workspace_root, updated)
        if retry_error:
            updated["current_phase"] = "run"
            updated["phase_status"] = "waiting_review"
            updated["execution_summary_status"] = "BLOCKED"
            updated["last_error"] = retry_error
            updated["next_actions"] = ["修正执行摘要阻断项后回复 重试执行摘要"]
        else:
            report_path = f".superlooper/reports/{updated['task_id']}/execution_summary.md"
            updated["current_phase"] = "run"
            updated["phase_status"] = "pending"
            updated["execution_summary_status"] = "NOT_STARTED"
            updated["execution_summary_report"] = None
            updated["invalidated_artifacts"] = append_invalidated_artifacts(updated, [report_path])
            updated["last_error"] = None
            updated["next_actions"] = [f"重新运行 /spl:run {updated['task_id']} 生成 execution_summary.md"]
        ...
        if review_error:
            ...
            report_path = f".superlooper/reports/{updated['task_id']}/code_review_report.md"
            updated["current_phase"] = "run"
            updated["phase_status"] = "pending"
            ...
            updated["invalidated_artifacts"] = append_invalidated_artifacts(
                ...
                [
                    report_path,
                    f".superlooper/reports/{updated['task_id']}/merge_report.json",
                    f".superlooper/reports/{updated['task_id']}/test_report.md",
                    f".superlooper/reports/{updated['task_id']}/apply_report.json",
                    f".superlooper/reports/{updated['task_id']}/session_report.md",
                    f".superlooper/reports/{updated['task_id']}/requirement_alignment_report.md",
                ],
            )
            updated["last_error"] = None
        ...
        if test_error:
            ...
            report_path = f".superlooper/reports/{updated['task_id']}/test_report.md"
            updated["current_phase"] = "run"
            updated["phase_status"] = "pending"
            updated["rollback_target_phase"] = "run"
            updated["affected_modules"] = []
        ...
        if conflict_error:
            ...
            updated["invalidated_artifacts"] = append_invalidated_artifacts(
                updated,
                [
                    f".superlooper/reports/{updated['task_id']}/apply_conflict_report.json",
                    f".superlooper/reports/{updated['task_id']}/apply_report.json",
                    f".superlooper/reports/{updated['task_id']}/session_report.md",
                    f".superlooper/reports/{updated['task_id']}/requirement_alignment_report.md",
                ],
        ...
        if impact_error:
            ...
            affected_modules = impact_data.get("affected_modules") or []
            updated["current_phase"] = target_phase
            updated["phase_status"] = "pending"
            updated["rollback_target_phase"] = target_phase
            updated["affected_modules"] = affected_modules
            updated["change_impact_report"] = None
            updated["invalidated_artifacts"] = append_invalidated_artifacts(updated, affected_artifacts)
            updated["last_error"] = None
            updated["next_actions"] = [
                f"只重跑 {target_phase} 阶段和受影响模块：{', '.join(affected_modules)}"
            ]
        ...
        if approval_error:
            ...
            updated["affected_modules"] = []
            updated["last_error"] = None
            updated["next_actions"] = [f"查看 .superlooper/reports/{updated['task_id']}/session_report.md"]
    write_state(state_path, updated)
    append_event_log(workspace_root, updated)
...
def print_normalized_action(workspace_root, state, normalized):
    ...
    if action == "approve_and_proceed":
        print(f"next_step: /spl:ui {task_id}")
        return
    if action == "ui_revision_requested":
    ...
    if action == "approve_ui_and_proceed":
        if state.get("ui_artifacts_validated") is not True:
            ...
            return
        print(f"next_step: /spl:design {task_id}")
        return
    ...
    if action == "execution_summary_revision_requested":
        ...
        return
    if action == "retry_execution_summary":
        if state.get("phase_status") == "pending":
            print(f"next_step: /spl:run {task_id}，重新生成 execution_summary.md。")
        else:
            print("next_step: 修正执行摘要阻断项后再次回复 重试执行摘要。")
        return
    if action == "execution_summary_approved":
        print(f"next_step: /spl:run {task_id}，按执行摘要确认结果启动并行开发。")
        return
    if action == "start_execution":
        print(f"next_step: /spl:run {task_id}，按当前执行清单启动并行开发。")
        return
...
def print_mapped_next_step(workspace_root, state):
    ...
    if current_phase == "prd" and phase_status == "pending":
        print(f"next_step: /spl:prd {requirement_path} {task_id}")
        return
    ...
    if current_phase == "ui_design" and phase_status in {"blocked", "failed"}:
        print("next_step: 查看 UI 阶段错误或阻塞原因，修复后重新运行 /spl:ui <task_id>。")
        return
    ...
    if current_phase == "run" and phase_status == "pending":
        print(f"next_step: /spl:run {task_id}")
        return
    ...
    if current_phase == "run" and phase_status == "running":
        ...
        return
    if current_phase == "run" and phase_status == "blocked":
        conflict_report = workspace_root / ".superlooper" / "reports" / task_id / "apply_conflict_report.json"
        apply_report = workspace_root / ".superlooper" / "reports" / task_id / "apply_report.json"
    ...
    if current_phase == "report" and phase_status == "passed":
        report_path = workspace_root / ".superlooper" / "reports" / task_id / "session_report.md"
...
def main():
    ...
    try:
        ...
        print(f"恢复 task 失败：{exc}", file=sys.stderr)
        return 1
