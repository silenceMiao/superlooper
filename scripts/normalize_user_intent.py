...
EXECUTION_SUMMARY_RETRY_KEYWORDS = ["重试执行摘要", "重新生成执行摘要", "阻断已修复，重试执行摘要", "阻断已修复，重新生成摘要"]
...
UI_REVISION_KEYWORDS = ["ui不", "ui 不", "ui有问题", "ui 有问题", "界面不", "页面不", "页面要改", "界面要改", "交互不", "视觉不", "预览不", "原型不", "颜色不", "字体不", "图标不", "导航不", "搜索不", "页面跳转不", "重新设计ui", "重新设计 ui"]
UI_APPROVE_KEYWORDS = ["ui通过", "ui 通过", "界面通过", "页面通过", "预览通过", "原型通过", "进入系统设计"]
REVIEW_NEGATION_KEYWORDS = ["不通过", "未通过", "尚未通过", "没有通过", "未确认", "不可以", "暂不通过", "不能通过", "不同意", "不确认", "先别", "暂不", "不要", "别执行", "不执行", "未执行", "暂停"]
...
CANONICAL_REPLIES = {
    ...
    "retry_execution_summary": "重试执行摘要",
    "start_execution": "执行",
...
def normalize_user_intent(state, user_input):
    ...
    if state.get("current_phase") == "report" and state.get("phase_status") == "passed":
        return result("show_status", "high", feedback, False, [], "task 已完成，只返回状态。")
    ...
    if current_phase == "prd" and phase_status == "waiting_review":
        ...
        if contains_any(normalized, REVIEW_NEGATION_KEYWORDS):
            return choice(feedback, ["prd_revision_requested", "show_status"], "PRD 审核阶段检测到否定或暂停语义，禁止自动批准。")
    ...
    if current_phase == "ui_design" and phase_status == "waiting_review":
        ...
        if contains_any(normalized, REVIEW_NEGATION_KEYWORDS):
            return choice(feedback, ["ui_revision_requested", "prd_revision_requested", "show_status"], "UI 审核阶段检测到否定或暂停语义，禁止自动批准。")
    ...
    if current_phase == "design" and phase_status == "waiting_review":
        ...
        if contains_any(normalized, REVIEW_NEGATION_KEYWORDS):
            options = ["change_impact_requested", "show_status"] if project_initialized else ["design_revision_requested", "show_status"]
            return choice(feedback, options, "设计审核阶段检测到否定或暂停语义，禁止自动批准。")
    ...
    if current_phase == "requirement_alignment" and phase_status == "waiting_review":
        if contains_any(normalized, ALIGNMENT_REVISION_KEYWORDS):
            return result("requirement_alignment_revision_requested", "high", feedback, False, [], "用户确认需求反向校对未通过，需要进入受控修正。")
        if contains_any(normalized, REVIEW_NEGATION_KEYWORDS):
            return choice(feedback, ["requirement_alignment_revision_requested", "show_status"], "需求反向校对阶段检测到否定或暂停语义，禁止自动批准。")
    ...
    if (
        current_phase == "run"
        and phase_status == "waiting_review"
        and state.get("execution_summary_status") == "BLOCKED"
        and contains_any(normalized, EXECUTION_SUMMARY_RETRY_KEYWORDS)
    ):
        return result("retry_execution_summary", "high", feedback, False, [], "用户明确要求在修复阻断项后重新生成执行摘要。")
    ...
    if current_phase == "run" and phase_status == "waiting_review" and state.get("execution_summary_status") == "READY_FOR_APPROVAL":
        ...
        if contains_any(normalized, REVIEW_NEGATION_KEYWORDS):
            return choice(feedback, ["execution_summary_revision_requested", "show_status"], "执行摘要审核阶段检测到否定或暂停语义，禁止自动批准。")
    ...
    if current_phase == "run" and phase_status == "waiting_review":
        if contains_any(normalized, REVIEW_NEGATION_KEYWORDS):
            return choice(feedback, ["change_impact_requested", "show_status"], "运行审核阶段检测到否定或暂停语义，禁止自动执行。")
    ...
    if contains_any(normalized, REVIEW_NEGATION_KEYWORDS):
        return choice(feedback, ["show_status", "continue_current_flow"], "检测到否定或暂停语义，禁止自动继续。")

    if contains_any(normalized, CONTINUE_KEYWORDS):
        return result("continue_current_flow", "high", feedback, False, [], "用户要求沿当前 active task 继续。")

    if contains_any(normalized, AMBIGUOUS_KEYWORDS):
        return choice(feedback, ["continue_current_flow", "show_status"], "用户输入存在歧义。")

    return result("unknown", "low", feedback, True, ["continue_current_flow", "show_status"], "无法根据当前 task state 识别用户意图。")
