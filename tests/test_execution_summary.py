...
class ExecutionSummaryScriptTest(unittest.TestCase):
    ...
    def read_state(self):
        return json.loads((self.workspace_root / ".superlooper" / "state" / f"{self.task_id}.json").read_text(encoding="utf-8"))

    def write_state(self, state):
        (self.workspace_root / ".superlooper" / "state" / f"{self.task_id}.json").write_text(json.dumps(state, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    ...
    def prepare_ready_workspace(self):
        ...
        manifest = self.run_script("generate_execution_manifest.py", "--workspace-root", str(self.workspace_root), "--task-id", self.task_id)
        self.assertEqual(manifest.returncode, 0, manifest.stderr)
        agents = self.run_script("generate_runtime_agents.py", "--workspace-root", str(self.workspace_root), "--task-id", self.task_id, "--agents-dir", "agents")
    ...
    def test_build_execution_summary_writes_report_and_updates_state(self):
        ...
        result = self.run_script("build_execution_summary.py", "--workspace-root", str(self.workspace_root), "--task-id", self.task_id)

        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn(f".superlooper/reports/{self.task_id}/execution_summary.md", result.stdout)
        ...
        self.assertEqual(state["execution_summary_report"], f".superlooper/reports/{self.task_id}/execution_summary.md")
    ...
    def test_build_execution_summary_report_passes_contract_validator(self):
        ...
        validate = self.run_script("validate_miao_contracts.py", "--workspace-root", str(self.workspace_root), "--task-id", self.task_id, "--scope", "execution-summary")
    ...
    def test_build_execution_summary_blocks_when_upstream_alignment_is_blocked(self):
        ...
        self.assertNotIn("通过：回复 `按此执行`", report)
        state = self.read_state()
        self.assertEqual(state["current_phase"], "run")
        self.assertEqual(state["phase_status"], "waiting_review")
        self.assertEqual(state["execution_summary_status"], "BLOCKED")

    def test_retry_blocked_execution_summary_after_prerequisites_are_fixed(self):
        self.prepare_ready_workspace()
        self.write_upstream_alignment(status="BLOCKED", blocking_decisions=["DEC-001 需要用户确认模块边界"])
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("canonical_action: retry_execution_summary", result.stdout)
        state = self.read_state()
        self.assertEqual(state["current_phase"], "run")
        ...
        self.assertIn(
            f".superlooper/reports/{self.task_id}/execution_summary.md",
            state["invalidated_artifacts"],
        )

    def test_retry_blocked_execution_summary_stays_blocked_when_prerequisites_fail(self):
        self.prepare_ready_workspace()
        self.write_upstream_alignment(status="BLOCKED", blocking_decisions=["DEC-001 需要用户确认模块边界"])
        blocked = self.run_script(
            "build_execution_summary.py",
        ...
        self.assertEqual(state["phase_status"], "waiting_review")
        self.assertEqual(state["execution_summary_status"], "BLOCKED")
        self.assertIn("DEC-001", state["last_error"])

    def test_build_execution_summary_accepts_legacy_cli_alias(self):
        self.prepare_ready_workspace()
